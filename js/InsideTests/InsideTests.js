...
// et lancer les tests
// const INSIDE_TESTS_ON = true ; // tests à en marche
...
class Test {
  ...
  static addTest(mainfolder, paire){
    ...
    this.promisesList.push(loadJS(`${folderpath}/${file}`))
  }
  static start(){
    try {
      Ajax.send('InsideTests/before-all.rb')
      .then(this.init.bind(this))
      .then(this.loadAllTestFiles.bind(this))
      .then(this.runNext.bind(this))
      .catch(err => {
        console.error(err)
        Ajax.send('InsideTests/after-all.rb')
      })
    } catch (e) {
  ...
  static runNext(){
    ...
    if ( nextTest ) {
      ...
      this.report()
      return Ajax.send('InsideTests/after-all.rb')
    }
  }
  static report(){
    var style = (() => {
